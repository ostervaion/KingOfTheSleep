...
env: ## Copy .env.example to .env (skips if .env already exists)
	@if [ -f $(ENV_FILE) ]; then \
		...
		echo "$(GREEN)Created $(ENV_FILE) from .env.example$(RESET)"; \
	fi
...
build-no-cache: ## Build all images without using cache
	@echo "$(CYAN)Building all images (no cache)…$(RESET)"
	$(COMPOSE) -f $(COMPOSE_FILE) build --no-cache

.PHONY: build-db
build-db: ## Build only the db image
...
restart-server: ## Restart only the Caddy container
...
# ── Database helpers ─────────────────────────────────────────
.PHONY: db-dump
db-dump: ## Dump the database to ./db/backup.sql
	@echo "$(CYAN)Dumping database…$(RESET)"
	$(COMPOSE) -f $(COMPOSE_FILE) exec db \
		pg_dump -U $${POSTGRES_USER:-appuser} $${POSTGRES_DB:-appdb} \
		> db/backup.sql
	@echo "$(GREEN)Dump saved to db/backup.sql$(RESET)"

.PHONY: db-restore
db-restore: ## Restore database from ./db/backup.sql
	@echo "$(CYAN)Restoring database from db/backup.sql…$(RESET)"
...
clean: down ## Remove containers, images, and build cache for this project
	@echo "$(YELLOW)Removing project images and build cache…$(RESET)"
