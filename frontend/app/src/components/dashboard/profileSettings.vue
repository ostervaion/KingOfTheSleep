...
async function loadUser() {
  ...
  try {
    ...
    if (fetchedUser?.avatar_path) {
      previewSrc.value = fetchedUser.avatar_path
    }
  } catch (error) {
    console.error(
...
async function onFileSelected(event) {
  ...
  try {
    ...
    if (!avatarUrl) {
      throw new Error(
        'The server did not return an avatar URL',
      )
    }

    previewSrc.value = avatarUrl
...
async function confirmDeleteAccount() {
  ...
  if (!username) {
    deleteError.value =
      'The username could not be loaded'
    return
  }
  ...
  <div
    ...
    <div class="px-4 pb-1 pt-4 sm:px-6 md:px-8 md:pb-2 md:pt-5">
      <div class="flex items-start justify-end gap-4">
        <button type="button" aria-label="Close"
          class="rounded-full px-2 text-lg leading-none text-neutral-400 transition hover:text-white" @click="onClose">
          ×
        </button>
      </div>
    </div>
    ...
    <div class="min-h-0 flex-1 overflow-y-auto px-4 pb-8 sm:px-6 md:px-8 md:pb-10">
      <template v-if="!showDeleteConfirm">
        ...
        <div class="flex flex-col items-center gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex flex-col items-center gap-3 sm:flex-row sm:items-center">
            <div class="relative h-24 w-24 sm:h-28 sm:w-28">
              <img :src="previewSrc" alt="Profile picture"
              ...
              <div v-if="isUploading || isLoadingUser" class="absolute inset-0 flex items-center justify-center">
          ...
          <div class="flex w-full flex-col gap-2 sm:w-auto sm:flex-row">
            <input ref="fileInput" type="file" accept=".jpg,.jpeg,.png,.webp,.gif,image/*" class="hidden"
        ...
        <form id="profile-form" class="space-y-4" @submit.prevent="onSaveChanges">
          <div class="grid grid-cols-1 gap-4 md:grid-cols-2">
            <!-- Username cargado desde /me -->
            <label class="block">
            ...
            <label class="block">
              ...
              <input v-model.trim="form.email" :disabled="isLoadingUser" type="email" autocomplete="email"
                class="inputField disabled:cursor-not-allowed disabled:opacity-50" placeholder="email@example.com" />
            </label>
          </div>
          ...
          <div class="rounded-lg bg-white/[0.02] p-3 sm:p-4">
            ...
            <div class="grid grid-cols-1 gap-4 md:grid-cols-3">
              <label class="block">
                ...
                <input v-model="form.currentPassword" type="password" autocomplete="current-password" class="inputField"
                  placeholder="••••••••" />
              </label>
              ...
              <label class="block">
                ...
                <input v-model="form.newPassword" type="password" autocomplete="new-password" class="inputField"
                  placeholder="••••••••" />
              </label>
              ...
              <label class="block">
                ...
                <input v-model="form.confirmPassword" type="password" autocomplete="new-password" class="inputField"
                  placeholder="••••••••" />
              </label>
            </div>
          </div>
          ...
          <p v-if="saveSuccess" class="text-xs text-emerald-400">
      ...
      <div v-else class="flex flex-col items-center gap-4 px-2 py-6 text-center sm:py-10">
        <img src="/DeleteAccount.png" alt="Sorry to see you go"
        ...
        <p class="max-w-sm text-sm leading-relaxed text-neutral-400">
          ...
          All your data, progress, and history will be
    ...
    <div v-if="!showDeleteConfirm"
      ...
      <button type="button" class="rounded-md px-3 py-2 text-xs font-medium text-red-400 transition hover:bg-red-950/20"
        @click="onDeleteAccount">
        Delete Account
      </button>

      <button type="submit" form="profile-form" :disabled="isSaving || isLoadingUser"
